...
void wipeListScreen() {
	...
	if (currentChannel.videoPid == -1) {
		...
		DFBCHECK(
				primary->FillRectangle(primary, 0, 0, screenWidth,
						screenHeight));
		...
		DFBCHECK(
				primary->FillRectangle(primary, screenWidth * 3 / 4, 0,
...
void wipeVolScreen() {
	...
	if (currentChannel.videoPid == -1) {
		...
		DFBCHECK(primary->SetColor(primary, 0x00, 0x00, 0x00, 0xff));
		DFBCHECK(
				primary->FillRectangle(primary, screenWidth / 10,
						screenHeight / 10, screenWidth / 10 + 200,
						screenHeight / 10 + 200));
...
void wipeScreen(/*union sigval signalArg*/) {
	...
	if (currentChannel.videoPid == -1) {
		...
		DFBCHECK(primary->SetColor(primary, 0x00, 0x00, 0x00, 0xff));
		DFBCHECK(
				primary->FillRectangle(primary, 0, screenHeight * 5 / 6,
...
uint16_t initRb() {
	...
	DFBCHECK(
			dfbInterface->CreateFont(dfbInterface,
					"/home/galois/fonts/DejaVuSans.ttf", &fontDesc,
					&fontInterface));
...
void* drawingEpg() {
	...
	if (currentChannel.videoPid == -1) {
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, screenWidth / 3,
						screenHeight / 6, DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 50,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1480, 120,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 100,
						DSTF_CENTER));
		sprintf(keycodeString, "Service: %s", channelList[0].serviceName);

		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 150,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1480, 220,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 200,
						DSTF_CENTER));
		sprintf(keycodeString, "Service: %s", channelList[1].serviceName);

		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 250,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1480, 320,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 300,
						DSTF_CENTER));
		...
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 350,
						DSTF_CENTER));
		sprintf(keycodeString, "%s", " CH4");

		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1480, 420,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 400,
						DSTF_CENTER));
		sprintf(keycodeString, "Service: %s", channelList[3].serviceName);

		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 450,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1480, 520,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 500,
						DSTF_CENTER));
		sprintf(keycodeString, "Service: %s", channelList[4].serviceName);

		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 550,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1480, 620,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 600,
						DSTF_CENTER));
		sprintf(keycodeString, "Service: %s", channelList[5].serviceName);

		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 650,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1480, 720,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 700,
						DSTF_CENTER));
		sprintf(keycodeString, "Service: %s", channelList[6].serviceName);

		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0x55));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, 1700, 750,
						DSTF_CENTER));
...
void* drawingVol() {
	...
	if (currentChannel.videoPid == -1) {
		...
		case 0:
			DFBCHECK(
					dfbInterface->CreateImageProvider(dfbInterface,
							"volume_0.png", &provider))
			;
			break;
		...
		case 2:
			DFBCHECK(
					dfbInterface->CreateImageProvider(dfbInterface,
							"volume_2.png", &provider))
			;
			break;
		case 3:
			DFBCHECK(
					dfbInterface->CreateImageProvider(dfbInterface,
							"volume_3.png", &provider))
			;
			break;
		case 4:
			DFBCHECK(
					dfbInterface->CreateImageProvider(dfbInterface,
							"volume_4.png", &provider))
			;
			break;
		case 5:
			DFBCHECK(
					dfbInterface->CreateImageProvider(dfbInterface,
							"volume_5.png", &provider))
			;
			break;
		case 6:
			DFBCHECK(
					dfbInterface->CreateImageProvider(dfbInterface,
							"volume_6.png", &provider))
			;
			break;
		case 7:
			DFBCHECK(
					dfbInterface->CreateImageProvider(dfbInterface,
							"volume_7.png", &provider))
			;
			break;
		case 8:
			DFBCHECK(
					dfbInterface->CreateImageProvider(dfbInterface,
							"volume_8.png", &provider))
			;
			break;
		case 9:
			DFBCHECK(
					dfbInterface->CreateImageProvider(dfbInterface,
							"volume_9.png", &provider))
			;
			break;
		case 10:
			DFBCHECK(
					dfbInterface->CreateImageProvider(dfbInterface,
							"volume_10.png", &provider))
			;
			break;
		...
		DFBCHECK(
				dfbInterface->CreateSurface(dfbInterface, &surfaceDesc,
						&logoSurface));
		...
		DFBCHECK(
				...
				/*source region, NULL to blit the whole surface*/NULL,
						/*destination x coordinate of the upper left corner of the image*/screenWidth
								/ 10,
						/*destination y coordinate of the upper left corner of the image*/screenHeight
								/ 10));
...
void* drawingBanner() {
	...
	if (currentChannel.videoPid == -1) {
		...
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1,
						screenWidth / 16, screenHeight * 11 / 12, DSTF_CENTER));
		...
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1,
						screenWidth / 16 + 50, screenHeight * 11 / 12,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0xff));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, screenWidth / 6,
						screenHeight * 11 / 12, DSTF_CENTER));
		...
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1,
						screenWidth / 6 + 140, screenHeight * 11 / 12,
						DSTF_CENTER));
		...
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1,
						screenWidth / 3 + 140, screenHeight * 11 / 12,
						DSTF_CENTER));
		...
		DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0xff));
		DFBCHECK(
				primary->DrawString(primary, keycodeString, -1, screenWidth / 2,
						screenHeight * 11 / 12, DSTF_CENTER));
		if (txt == true) {
			DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0xff));
			DFBCHECK(
					primary->DrawString(primary, "YES", -1,
							screenWidth / 2 + 120, screenHeight * 11 / 12,
							DSTF_CENTER));
		} else {
			DFBCHECK(primary->SetColor(primary, 0xff, 0xff, 0xff, 0xff));
			DFBCHECK(
					primary->DrawString(primary, "NO", -1,
							screenWidth / 2 + 120, screenHeight * 11 / 12,
							DSTF_CENTER));
		}
...
StreamControllerError startChannel(int32_t channelNumber) {
	...
	if (Demux_Set_Filter(playerHandle,
			patTable->patServiceInfoArray[channelNumber + 1].pid, 0x02,
			&filterHandle)) {
	...
	for (i = 0; i < pmtTable->elementaryInfoCount; i++) {
		if (((pmtTable->pmtElementaryInfoArray[i].streamType == 0x1)
				|| (pmtTable->pmtElementaryInfoArray[i].streamType == 0x2)
				|| (pmtTable->pmtElementaryInfoArray[i].streamType == 0x1b))
				&& (videoPid == -1)) {
			...
			firstV = 1;
		} else if (((pmtTable->pmtElementaryInfoArray[i].streamType == 0x3)
				|| (pmtTable->pmtElementaryInfoArray[i].streamType == 0x4))
				&& (audioPid == -1)) {
			...
			if (firstA == 0) {
				if (audioPid != argAudioPid) {
					printf(
							"ERROR: Wrong audio PID\nPressed exit and try again with new audio PID\n");
					return SC_ERROR;
				}
				if (argAudioType
						!= 10 /*|| argAudioType!=11 || argAudioType!=12 || argAudioType!=1*/) {
					if (argAudioType != 11) {
						if (argAudioType != 12) {
							if (argAudioType != 1) {
								printf(
										"ERROR: Wrong audio TYPE\nPressed exit and try again with new video TYPE\n");
								return SC_ERROR;
							}
	...
	if (videoPid != -1) {
		...
		/* create video stream */
		if (Player_Stream_Create(playerHandle, sourceHandle, videoPid,
				argVideoType, &streamHandleV)) {
	...
	if (audioPid != -1) {
		...
		/* create audio stream */
		if (Player_Stream_Create(playerHandle, sourceHandle, audioPid,
				argAudioType, &streamHandleA)) {
...
void* streamControllerTask(argStruct* arg_struct) {
	...
	/* lock to frequency */
	if (!Tuner_Lock_To_Frequency(arg_struct->frequency, arg_struct->bandwidth,
			arg_struct->modul)) {
	...
	if (tableId == 0x00) {
		...
		if (parseSdtTable(buffer, sdtTable) == TABLES_PARSE_OK) {
			...
			for (i = 0; i < 7; i++) {
				...
				strcpy(channelList[i].providerName,
						sdtTable->sdtElementaryInfoArray[i].descriptor.providerName);
				strcpy(channelList[i].serviceName,
						sdtTable->sdtElementaryInfoArray[i].descriptor.serviceName);
