...
ParseErrorCode parsePatTable(const uint8_t* patSectionBuffer,PatTable* patTable) {
	...
	if (parsePatHeader(patSectionBuffer, &(patTable->patHeader))
			!= TABLES_PARSE_OK) {
...
ParseErrorCode parsePmtTable(const uint8_t* pmtSectionBuffer,PmtTable* pmtTable) {
	...
	if (parsePmtHeader(pmtSectionBuffer, &(pmtTable->pmtHeader))
			!= TABLES_PARSE_OK) {
		printf("\n%s : ERROR parsing PMT header\n", __FUNCTION__);
		return TABLES_PARSE_ERROR;
	}

	parsedLength = 12 + pmtTable->pmtHeader.programInfoLength /*PMT header size*/
	+ 4 /*CRC size*/- 3 /*Not in section length*/;
	currentBufferPosition = (uint8_t *) (pmtSectionBuffer + 12
			+ pmtTable->pmtHeader.programInfoLength); /* Position after last descriptor */
	...
	while (parsedLength < pmtTable->pmtHeader.sectionLength) {
		if (pmtTable->elementaryInfoCount
				> TABLES_MAX_NUMBER_OF_ELEMENTARY_PID - 1) {
			printf(
		...
		if (parsePmtElementaryInfo(currentBufferPosition,
				&(pmtTable->pmtElementaryInfoArray[pmtTable->elementaryInfoCount]))
				== TABLES_PARSE_OK) {
			currentBufferPosition +=
					5
							+ pmtTable->pmtElementaryInfoArray[pmtTable->elementaryInfoCount].esInfoLength; /* Size from stream type to elemntary info descriptor*/
			parsedLength +=
					5
							+ pmtTable->pmtElementaryInfoArray[pmtTable->elementaryInfoCount].esInfoLength; /* Size from stream type to elementary info descriptor */
...
ParseErrorCode parseSdtServiceInfo(const uint8_t* sdtServiceInfoBuffer,SdtElementaryInfo* sdtServiceInfo) {
	...
	if (sdtServiceInfo->runningStatus == 0x04)	//if  the service is running
		...
		for (k = 0; k < sdtServiceInfo->descriptorLoopLength;) {
			...
			sdtServiceInfo->descriptor.descriptorTag = (uint8_t)(
					*(sdtServiceInfoBuffer + 5 + k));
			sdtServiceInfo->descriptor.descriptorLength = (uint8_t)(
					*(sdtServiceInfoBuffer + 6 + k));
			...
			if (sdtServiceInfo->descriptor.descriptorTag != 0x48) {
				...
				printf("usao u else\n");
				sdtServiceInfo->descriptor.serviceType = (uint8_t)(
						*(sdtServiceInfoBuffer + 7 + k));
				sdtServiceInfo->descriptor.serviceProviderNameLength =
						(uint8_t)(*(sdtServiceInfoBuffer + 8 + k));

				for (i = 0;
						i < sdtServiceInfo->descriptor.serviceProviderNameLength;
						i++) {
					//one char is one byte
					sdtServiceInfo->descriptor.providerName[i] =
							(char) (*(sdtServiceInfoBuffer + 9 + k + i));
				}
				sdtServiceInfo->descriptor.providerName[sdtServiceInfo->descriptor.serviceProviderNameLength] =
						'\0';
				//printf("\nProviderName: %s \n",sdtServiceInfo->descriptor.providerName);
				sdtServiceInfo->descriptor.serviceNameLegth =
						(uint8_t)(
								*(sdtServiceInfoBuffer + 9 + k
										+ sdtServiceInfo->descriptor.serviceProviderNameLength));
				for (i = 0; i < sdtServiceInfo->descriptor.serviceNameLegth;
						i++) {
					sdtServiceInfo->descriptor.serviceName[i] =
							(char) (*(sdtServiceInfoBuffer + 9 + k
									+ sdtServiceInfo->descriptor.serviceProviderNameLength
									+ i + 1));
				}
				sdtServiceInfo->descriptor.serviceName[sdtServiceInfo->descriptor.serviceNameLegth] =
						'\0';
...
ParseErrorCode parseSdtTable(const uint8_t* sdtSectionBuffer,SdtTable* sdtTable) {
	...
	if (parseSdtHeader(sdtSectionBuffer, &(sdtTable->sdtHeader))
			!= TABLES_PARSE_OK) {
	...
	for (; parsedLength < sdtTable->sdtHeader.sectionLength;)
		if (sdtTable->elementaryInfoCount > TABLES_MAX_NUMBER_OF_SDT_PID - 1) {
			printf(
					...
					__FUNCTION__);
			return TABLES_PARSE_ERROR;
		} else if (parseSdtServiceInfo(currentBufferPosition,
				&(sdtTable->sdtElementaryInfoArray[sdtTable->elementaryInfoCount]))
				== TABLES_PARSE_OK) {
			printf("\nstigao na kraj parseSDTInfo\n");
			currentBufferPosition +=
					5
							+ sdtTable->sdtElementaryInfoArray[sdtTable->elementaryInfoCount].descriptorLoopLength;
			parsedLength +=
					5
							+ sdtTable->sdtElementaryInfoArray[sdtTable->elementaryInfoCount].descriptorLoopLength;
